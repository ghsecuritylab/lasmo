...
void lsm_galva_init(void){
  palSetPadMode(GPIOA, PORTDAC1, PAL_MODE_INPUT_ANALOG);
  palSetPadMode(GPIOA, PORTDAC2, PAL_MODE_INPUT_ANALOG);
  dacObjectInit(&DACD1);
  dacStart(&DACD1, &dac1cfg1);
}

//This function allows to print a line depending on the value on the axis x
//and each point of this line is printed every 30 microsecondes in order to see a fix line.
// In 0.03 seconds, we need to print one frame.
...
static dacsample_t dac_buffer_sin[DAC_BUFFER_SIZE]= {
...
static void end_cb(DACDriver *dacp, dacsample_t *buffer, size_t n) {
  (void)dacp;
  nz++;
...
static const DACConversionGroup dacgrp1 = {
  .num_channels = 1U,
  .end_cb       = end_cb,
  .error_cb     = error_cb,
  .trigger      = DAC_TRG(0)
...
void lsm_sin_ctrl_galva(void){
  ...
  SEGGER_RTT_printf(0, "Begin of the sinusoide \r\n");
  dacStartConversion(&DACD1, &dacgrp1,(dacsample_t *)dac_buffer_sin, DAC_BUFFER_SIZE);
